...
import { Home, AddToPhotos, Inventory, Archive, HomeOutlined, AddToPhotosOutlined, Inventory2Outlined as InventoryOutlined, ArchiveOutlined } from '@mui/icons-material';
...
export default function Header(){
    ...
    return (
        <>
            ...
            :
            <Paper sx={{ backgroundColor: '#171717', position: 'fixed', bottom: 0, paddingTop: 3, right: 0, left: 0 }} elevation={3}>
                <BottomNavigation sx={{ backgroundColor: '#171717', paddingBottom: isMobile ? 5 : 0 }} value={location.pathname} onChange={(e, val) => {
                    navigate(val)
                }}>
                    ...
                    <BottomNavigationAction
                        label="Request"
                        showLabel
                        value="/inventory/restocks/create"
                        icon={location.pathname === '/inventory/restocks/create' ? <AddToPhotos /> : <AddToPhotosOutlined />}
                        style={{
                            color: 'white'
                        }}
                    />
                    <BottomNavigationAction
                        label="Restock"
                        showLabel
                        value="/inventory/restocks/view"
                        icon={location.pathname === '/inventory/restocks/view' ? <Archive /> : <ArchiveOutlined />}
                        style={{
                            color: 'white'
                        }}
                    />
                    <BottomNavigationAction
                        label="Pallets"
                        showLabel
                        value="/inventory/pallets"
                        icon={location.pathname === '/inventory/pallets' ? <Inventory /> : <InventoryOutlined />}
                        style={{
                            color: 'white'
                        }}
                    />
